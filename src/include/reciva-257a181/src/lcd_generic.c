...
void reciva_lcd_redraw(void)
{
  ...
  {
    driver->draw_screen(ppcTexts, iXcoord, iYcoord, iCursorTypeGlobal, 
...
static int draw_progress_simple(struct lcd_draw_progress_simple *dps)
{
  ...
  d = (double)dps->iProgress/(dps->iHighest - dps->iLowest);
  percent = d * 100;
  ...
  {
    if (!(ppcText[0] = kmalloc(i + 5, GFP_KERNEL)))
    {
      kfree(pcText);
      goto exit;
    }
  ...
  ret = driver->draw_screen(ppcText, 0, 0, LCD_CURSOR_OFF, NULL, NULL);
                            ...
                            int *upiLineContents)
{
  ...
  int ret = driver->draw_screen(ppcTexts, iX, iY, iCursorType,
                                piArrows, piLineContents);
