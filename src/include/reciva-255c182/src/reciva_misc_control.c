...
{
  printk(KERN_ERR "%s bit %d to GP%c%d\n", __FUNCTION__, bit, pin->c, pin->n);
  ...
  /* send data */
  rutl_regwrite(bit << pin->n, !bit << pin->n, GP(pin->c, DAT));
             ...
             unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd)
  {
    ...
    case IOC_RMC_PIN_WRITE:
      ...
      printk(KERN_ERR "IOC_RMC_PIN_WRITE J%d pin %d, data=%d\n", pc.connector, pc.pin, pc.data);

      pin = get_gpio_channel(pc.connector, pc.pin);
      if (!pin)
        return -EINVAL;

      printk(KERN_ERR "Using gpio channel GP%c%d\n", pin->c, pin->n);
